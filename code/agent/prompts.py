...
def _build_dynamic_search_space_description_1(search_space: dict) -> str:
...
- "param": The name of the hyperparameter. **IMPORTANT: Use the base name only** (e.g., "learning_rate", "optimizer"). You must specify whether to target the client or server space by including a `target` key.
- "key": The property to change ("min", "max", or "values").
- "value": The new value for that property.
- "target": Must be either "client_hps" or "server_hps".
...
def get_hp_suggestion_prompt_2(
    client_id: int,
    cluster_id: int,
    model_name: str,
    dataset_name: str,
...
def get_hp_suggestion_prompt_3(
    client_id: int, cluster_id: int, model_name: str, dataset_name: str,
    hpo_report: dict, search_space: dict, analysis_from_last_round: dict | None = None,
    peer_history: list | None = None, arc_cfg: int = 0, total_layers: int = 0
) -> str:

    # --- THIS IS THE FIX: Re-introducing the string formatting logic ---
    history_lines = []
    if hpo_report:
        for epoch, report in sorted(hpo_report.items()):
            hps_str = ", ".join(f"{k}={v}" for k, v in report.get('hps_suggested', {}).items())
    ...
    analysis_str = "No analysis from the previous round is available."
    if analysis_from_last_round:
        # Assuming last_analysis is a dictionary with a 'decision_summary'
        analysis_str = analysis_from_last_round.get('decision_summary', 'Analysis available but summary is missing.')
...
- **Peer History (This Epoch):**
{peer_history_str}
- **Last Round's Analysis:**
{analysis_str}
- **Client HPs to select:** `learning_rate`, `weight_decay`, `momentum`, `optimizer`, `scheduler`, `local_epochs`, `batch_size`, `dropout_rate`.
- **Your Task:** Based on all context, choose the best HPs for the client's local training. High `local_epochs` can cause client drift, which `mu` helps prevent.

{'-'*40}
**SERVER-SIDE CONTEXT & INSTRUCTIONS**
- **Model Split:** The client runs the first {arc_cfg} of {total_layers} layers. The server runs the remaining {total_layers - arc_cfg} layers.
- **Server Task:** The server model is deeper and more computationally demanding. It may benefit from a different `learning_rate` or `optimizer` than the client.
- **Server HPs to select:** `learning_rate`, `momentum`, `optimizer`, `scheduler`.

{'-'*40}
**OUTPUT FORMAT**
Return a single, valid JSON object with "reasoning" and "hps" keys. The "hps" object must contain "client", "server", and "mu" keys as shown in the example.
...
**ACTIONS SYNTAX:**
The "actions" key must be a list of JSON objects...
- "param": The name of the hyperparameter. **IMPORTANT: Use the base name only** (e.g., "learning_rate", "optimizer"), not nested paths like "client.learning_rate".
- "key": The property to change ("min", "max", or "values").
- "value": The new value.
